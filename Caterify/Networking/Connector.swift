...
struct Connector<N, S> where N: NetworkDescription, S: Codable {
    ...
    func doConnect(request: N, baseUrl: String) -> AnyPublisher<S, CABaseErrorModel> {
...
//                    print("debugNetwork: \(String(data: data, encoding: .utf8))")
                    ...
                    throw httpError(response.statusCode, errors.errors)
                }
                return data
            })
...
extension Connector {
    ...
    private func httpError(_ statusCode: Int, _ errors: ErrorModel?) -> CABaseErrorModel {
        switch statusCode {
        ...
        case 402, 405...499: return BaseError.Error4xx(status: statusCode, cache: nil, errors: errors).desc
